...
`PX4_INFO`相当于输出到PX4 shell的`printf`（包含在Firmware/src/platforms/px4_log.h中）。 这里有不同的日志级别：`PX4_INFO`、`PX4_WARN`、`PX4_ERR`、`PX4_DEBUG`。 警告和错误会额外添加到 ULog 并显示在 Flight Review 中。
